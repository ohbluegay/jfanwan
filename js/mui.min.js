/*!
 * =====================================================
 * Mui v2.1.0 (http://github.com/dcloudio/mui)
 * =====================================================
 */
...
function(a) {
	var b = 1,
		...
		k = /^(INPUT|TEXTAREA|BUTTON|SELECT)$/;
	a.fn.on = function(b, d, e) {
		return this.each(function() {
			...
			var n = m[d] || (m[d] = []);
			n.push(e), g && (i[h(f)] = j(f, b, d, e), f.addEventListener(b, i[h(f)]), "tap" === b && f.addEventListener("click", function(a) {
				if (a.target) {
		...
		return this.each(function() {
			...
			c[f] && a.isEmptyObject(c[f][b]) && (this.removeEventListener(b, i[f]), delete i[f])
		})
	}
}(mui),
function(a, b) {
	...
	var c, d, e = function(e, f) {
		...
		switch (e.type) {
			case a.EVENT_END:
				if (!f.isFinal) return;
				var i = g.target;
				if (!i || i.disabled || i.classList.contains("mui-disabled")) return;
...
function(a, b) {
	var c = function(c, d) {
		...
		switch (c.type) {
			...
			case a.EVENT_END:
			case a.EVENT_CANCEL:
				a.options.gestureConfig.pinch && f.pinch && 2 === d.touches.length && a.trigger(f.target, b + "end", d)
		}
	};
	a.addGesture({
		name: b,
...
function(a, b, c, d) {
	var e = "mui-scrollbar",
		...
		k = a.Class.extend({
			...
			_start: function(b) {
				if (this.moved = this.needReset = !1, this._transitionTime(), this.isInTransition && this.moved) {
					this.needReset = !0, this.isInTransition = !1;
					var c = a.parseTranslateMatrix(a.getStyles(this.scroller, "webkitTransform"));
					this.setTranslate(Math.round(c.x), Math.round(c.y)), this.resetPosition(), a.trigger(this.scroller, "scrollend", this), b.preventDefault()
				}
...
function(a, b, c, d) {
	var e = "mui-visibility",
		...
		g = a.Scroll.extend(a.extend({
			...
			_start: function(a) {
				this.loading || (this.pulldown = this.pullPocket = this.pullCaption = this.pullLoading = !1), a.preventDefault(), this._super(a)
			},
			_drag: function(a) {
			...
			resetPosition: function(a) {
				if (this.pulldown) {
					...
					this.topPocket.classList.remove(e)
				}
			...
			endPullupToRefresh: function(a) {
				var b = this;
				b.bottomPocket && b.loading && !this.pulldown && (b.loading = !1, a ? (this.finished = !0, b._setCaption(b.options.up.contentnomore), b.wrapper.removeEventListener("scrollbottom", b)) : (b._setCaption(b.options.up.contentdown), setTimeout(function() {
				...
				switch (b.type) {
					...
					case "dragend":
						if (this.isDragging) {
							...
							if (this.slideIn) {
								if (f = g >= 0 ? this.offCanvasRightWidth && g / this.offCanvasRightWidth || 0 : this.offCanvasLeftWidth && g / this.offCanvasLeftWidth || 0, f >= .5 && "left" === e ? this.openPercentage(0) : f > 0 && .5 >= f && "left" === e ? this.openPercentage(-100) : f >= .5 && "right" === e ? this.openPercentage(0) : f >= -.5 && 0 > f && "left" === e ? this.openPercentage(100) : f > 0 && .5 >= f && "right" === e ? this.openPercentage(-100) : -.5 >= f && "right" === e ? this.openPercentage(0) : f >= -.5 && "right" === e ? this.openPercentage(100) : -.5 >= f && "left" === e ? this.openPercentage(0) : f >= -.5 && "left" === e ? this.openPercentage(-100) : this.openPercentage(0), 1 === f || -1 === f || 0 === f) return void this._dispatchEvent()
							} else {
								...
								f > 0 && .5 > f && "right" === e ? this.openPercentage(0) : f > .5 && "left" === e ? this.openPercentage(100) : 0 > f && f > -.5 && "left" === e ? this.openPercentage(0) : "right" === e && 0 > f && f > -.5 ? this.openPercentage(0) : .5 > f && "right" === e ? this.openPercentage(-100) : "right" === e && f >= 0 && (f >= .5 || d.flick) ? this.openPercentage(100) : "left" === e && 0 >= f && (-.5 >= f || d.flick) ? this.openPercentage(-100) : this.openPercentage(0), (1 === f || -1 === f) && this._dispatchEvent()
							}
						}
				}
			},
...
function(a, b, c, d, e) {
	var f = "mui-control-item",
		...
		j = "mui-tab-item",
		k = function(a, b) {
			return b.classList && (b.classList.contains(f) || b.classList.contains(j)) ? (a.preventDefault(), b) : !1
		};
	a.registerTarget({
		name: d,
		index: 80,
		...
		var e = a.targets.tab;
		if (e) {
			for (var k, l, m, n = "mui-active", o = "." + n, p = e.parentNode; p && p !== c; p = p.parentNode) {
			...
			if (e && e.classList.add(n), e.hash && (m = c.getElementById(e.hash.replace("#", "")))) {
				if (!m.classList.contains(h)) return void e.classList[q ? "remove" : "add"](n);
				if (!q) {
					var r = m.parentNode;
	...
	var b = "ontouchstart" in document,
		...
		i = a.Numbox = a.Class.extend({
			init: function(b, c) {
				...
				d.holder = b, d.holder.__numbox_inited || (d.holder.__numbox_inited = !0, c = c || {}, c.step = parseInt(c.step || 1), d.options = c, d.input = a.qsa("." + h, d.holder)[0], d.plus = a.qsa("." + f, d.holder)[0], d.minus = a.qsa("." + g, d.holder)[0], d.checkValue(), d.initEvent())
			},
			initEvent: function() {
				var b = this;
		...
		return this.each(function(a, b) {
			if (d) new i(b, d);
				...
				var c = b.getAttribute("data-numbox-options"),
					d = c ? JSON.parse(c) : {};
				d.step = b.getAttribute("data-numbox-step") || d.step, d.min = b.getAttribute("data-numbox-min") || d.min, d.max = b.getAttribute("data-numbox-max") || d.max, new i(b, d)
			}
		}), this
	}, a.ready(function() {
		a("." + e).numbox()
...
var dateKey = "jfwlc.com20161219"+dateTime;
